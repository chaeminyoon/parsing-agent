...
비전 수리에는 환각 방어가 세 겹 있다. task가 들고 온 페이지에 라벨이 없으면 문서 전체에서 재탐색하고, 어디에도 없으면 crop 자체를 포기한다. 재구성된 셀의 절반 이상이 crop의 텍스트 레이어에 실존해야 패치되고, 미달이면 `content_mismatch`로 거부된다. 이 방어들은 골든셋 사람 라벨이 실제로 잡아낸 환각 사고에서 나왔다.
...
두 종류를 잰다. 채점기는 자체 결정적 메트릭이고, Parse-Everything은 이 채점기를 내부에서 최적화하므로 유리하다는 점은 감안하고 봐야 한다. 중립 검증은 사람 라벨로 하는 게 맞고, `golden/`에 그 프로토콜이 있다.
...
오픈소스 파서들과 같은 잣대로 쟀을 때. 실제 환경영향평가 PDF 3종 (2026-07-02 실측).
...
위 표는 2026-07-13의 교차 파서 융합·TOC 복원 반영 이전 실측이다. 반영 후 키리스 재실측에서는 같은 잣대로 추가 상승이 확인됐다 (예: NOAA NDBC 가이드 0.582 → 0.856, NOAA CO-OPS 사양서 0.676 → 0.765 — [benchmarks/results/RESULTS.md](benchmarks/results/RESULTS.md) 참고). 위 표의 문서별 세부 수치도 같은 파일에 있다.
